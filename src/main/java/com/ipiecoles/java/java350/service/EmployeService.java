...
public class EmployeService {
    ...
    public void calculPerformanceCommercial(String matricule, Long caTraite, Long objectifCa)
        ...
        appliquerBonusSiSuperieurMoyenne(employe, performance);

        employeRepository.save(employe);
    }

    private void verifierParametres(String matricule, Long caTraite, Long objectifCa)
            throws EmployeException {
    ...
    private Integer calculerPerformance(Employe employe, Long caTraite, Long objectifCa) {

        double ratio = (double) caTraite / objectifCa;
        int performance = Entreprise.PERFORMANCE_BASE;

        if (ratio >= 0.8 && ratio < 0.95) {
            performance = Math.max(Entreprise.PERFORMANCE_BASE,
                    employe.getPerformance() - 2);
    ...
    private void appliquerBonusSiSuperieurMoyenne(Employe employe, Integer performance) {

        Double moyenne = employeRepository
                .avgPerformanceWhereMatriculeStartsWith("C");

        if (moyenne != null && performance > moyenne) {
            employe.setPerformance(performance + 1);
        } else {
